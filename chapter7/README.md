...
---
### 7.1.1 함수형 인터페이스
* Ruannable이나 Callable같이 메서드 하나만을 가지는 인터페이스는 단일 추상 메서드(SAM (single abstract method))라고 부른다.
...
* 스트림은 여러모로 컬렉션과 비슷하지만 다음과 같은 중요한 차이점이 있다.
	...
	* 무한한 스트림이 가능하다. 일례로 모든 정수를 리턴하는 스트림을 만들어 limit()이나 findFirst() 같은 메서드를 사용하셔 그 부분집합을 구할 수 있다.
